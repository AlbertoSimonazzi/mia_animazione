...
export class PNGAnimationController {
    ...
    /**
     * Play animazione disassemblaggio (inverso)
     */
    async playReverseAnimation(onComplete, onComponentStart, onComponentComplete) {
        if (this.isPlaying) {
            console.log('⚠ Animazione già in corso, ignoro');
            return;
        }

        console.log('🔄 Inizio playReverseAnimation');
        this.isPlaying = true;

        // Inizia dalla posizione assemblata
        this.components.forEach(comp => {
            this.setPosition(comp, 0, 0);
        });

        // Ordine inverso: dall'ultimo al primo (LIFO - Last In First Out)
        for (let i = this.components.length - 1; i >= 0; i--) {
            if (!this.isPlaying) {
                console.log('⏹ Disassemblaggio fermato a componente', i);
                break;
            }
            ...
            console.log(`🔄 Componente ${i}: ${comp.config.name} (disassemblaggio)`);

            if (onComponentStart) {
                onComponentStart(i, comp.config.name);
            }

            // Anima verso la posizione esplosa
            await this.animateComponentReverse(comp);
        ...
        console.log('✅ Disassemblaggio completato');
        if (onComplete) {
            onComplete();
        }
    }
    ...
    /**
     * Anima un componente verso la posizione esplosa (reverse)
     */
    animateComponentReverse(comp) {
        return new Promise(resolve => {
            const startPos = { x: 0, y: 0 }; // Parte dalla posizione assemblata
            const endPos = comp.config.explodeDirection; // Va verso posizione esplosa
            const duration = this.config.animation.duration / this.speedMultiplier;
            const startTime = performance.now();
